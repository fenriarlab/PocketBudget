...
void main() {
  SavingsGoalModel goal(
          {required double currentAmount, required DateTime targetDate}) =>
      SavingsGoalModel(
        id: 'goal-1',
        title: '旅行基金',
        targetAmount: 10000,
  ...
  Widget buildScreen(
      {required bool privacyHidden, required SavingsGoalModel savingsGoal}) {
    return MaterialApp(
  ...
  testWidgets('plan screen shows grouped budget amounts and goal status',
      (tester) async {
  ...
  testWidgets('plan screen masks budget and goal amounts in privacy mode',
      (tester) async {
    await tester.pumpWidget(buildScreen(
        privacyHidden: true,
        savingsGoal:
            goal(currentAmount: 10000, targetDate: DateTime(2026, 8, 1))));
